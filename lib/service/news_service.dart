...
class NewsService extends GetxService {
  ...
  Future<void> fetchRecentNews() async {
    try {
      ...
      debugPrint(
          'NewsService: After removing duplicates: ${uniqueNews.length} articles');
  ...
  Future<List<Map<String, dynamic>>> _fetchFromRSSFeeds() async {
    try {
      ...
      debugPrint(
          'NewsService: Starting to fetch from ${rssUrls.length} RSS feeds');
      ...
      for (int i = 0; i < rssUrls.length; i++) {
        ...
        try {
          ...
          if (response.statusCode == 200) {
            ...
            debugPrint(
                'NewsService: Added ${rssNews.length} articles from $sourceName');
          } else {
            debugPrint(
                'NewsService: Failed to fetch from $sourceName - Status: ${response.statusCode}');
          }
        } catch (e) {
      ...
      debugPrint(
          'NewsService: Total RSS articles fetched: ${allRssNews.length}');
      return allRssNews;
    } catch (e) {
  ...
  List<Map<String, dynamic>> _parseRSSFeed(
      String xmlContent, String sourceUrl) {
    try {
      ...
      for (final pattern in patterns) {
        ...
        for (final itemMatch in itemMatches) {
          ...
          final contentMatch = RegExp(r'<content>(.*?)</content>', dotAll: true)
              .firstMatch(itemContent);
          final summaryMatch = RegExp(r'<summary>(.*?)</summary>', dotAll: true)
              .firstMatch(itemContent);
          ...
          // Extract link
          final linkMatch =
              RegExp(r'<link>(.*?)</link>').firstMatch(itemContent);
          ...
          final updatedMatch =
              RegExp(r'<updated>(.*?)</updated>').firstMatch(itemContent);
          ...
          final mediaContentMatch = RegExp(
                  r'<media:content[^>]*url=["\"](.*?)["\"][^>]*/?>',
                  dotAll: true)
              .firstMatch(itemContent);
          ...
          if (title.isNotEmpty &&
              (cleanDescription.isNotEmpty || title.length > 20)) {
            articles.add({
              'id': link.isNotEmpty
                  ? link
                  : '${sourceName}_${DateTime.now().millisecondsSinceEpoch}',
              'title': title,
              ...
              'content': cleanDescription.isNotEmpty
                  ? cleanDescription
            ...
            debugPrint(
                'NewsService: Added article from $sourceName: ${title.substring(0, title.length > 50 ? 50 : title.length)}...');
          }
        }
      ...
      debugPrint(
          'NewsService: Parsed ${articles.length} articles from $sourceName');
      return articles;
    } catch (e) {
      debugPrint(
          'NewsService: RSS parsing error for ${_getSourceName(sourceUrl)}: $e');
      return [];
    }
  }
  ...
  List<Map<String, dynamic>> _filterRecentNews(
      List<Map<String, dynamic>> articles) {
    final now = DateTime.now();
    final yesterday = now.subtract(const Duration(days: 1));

    debugPrint(
        'NewsService: Filtering ${articles.length} articles for recent news');
      ...
      try {
        ...
        if (articleDate == null && dateString.isNotEmpty) {
          ...
          try {
            ...
            debugPrint(
                'NewsService: Failed to parse RSS date format: $dateString');
          }
        }
        ...
        if (articleDate == null) {
          debugPrint(
              'NewsService: No valid date for article: ${article['title']} - Assuming recent');
          return true; // Include articles without dates for now
        }

        // Check if article is from today or yesterday (more lenient)
        final isRecent = articleDate
                .isAfter(yesterday.subtract(const Duration(hours: 24))) &&
            articleDate.isBefore(now.add(const Duration(hours: 12)));

        if (!isRecent) {
          debugPrint(
              'NewsService: Article too old: ${article['title']} - Date: $articleDate');
        }

        return isRecent;
      } catch (e) {
        debugPrint(
            'NewsService: Error parsing date for article: ${article['title']} - Error: $e');
        return true; // Include articles with parsing errors for now
      }
    }).toList();

    debugPrint(
        'NewsService: After filtering, ${filteredArticles.length} articles remain');
    ...
    newsArticles.assignAll([
      {
        ...
        'summary':
            'A major technology conference has been announced for next month, featuring industry leaders and innovative startups.',
        'content':
            'The conference will bring together technology leaders, entrepreneurs, and innovators from around the world. The event will feature keynote speeches, panel discussions, and networking opportunities.',
      ...
      },
      {
        ...
        'summary':
            'Global markets are showing signs of recovery as economic indicators improve across major economies.',
        'content':
            'Recent economic data suggests a positive trend in global markets. Analysts are optimistic about the recovery trajectory, though challenges remain in certain sectors.',
      ...
      },
      {
        ...
        'summary':
            'The championship finals have been scheduled for next week, with top teams competing for the ultimate prize.',
        'content':
            'After an exciting season, the championship finals are finally set. Fans are eagerly anticipating the showdown between the top-performing teams.',
        'author': 'Sports Desk',
      ...
      },
      {
        ...
        'summary':
            'Scientists have announced a breakthrough in medical research that could revolutionize treatment options.',
        'content':
            'The breakthrough involves a new approach to treating chronic conditions that affects millions of people worldwide. Clinical trials have shown promising results.',
      ...
      },
      {
        ...
        'summary':
            'The latest blockbuster movie has broken box office records in its opening weekend.',
        'content':
            'The film has received critical acclaim and has become the highest-grossing movie of the year so far. Fans are already asking for sequels.',
  ...
  List<Map<String, dynamic>> getNewsByCategory(String category) {
    ...
    if (category == 'All') {
      return newsArticles.toList(); // Show all articles for "All" category
    }
    return newsArticles
        .where((article) =>
  ...
  List<String> getAvailableCategories() {
    final categories = newsArticles
        .map((article) => article['category']?.toString() ?? 'Uncategorized')
        .toSet()
        .toList();
