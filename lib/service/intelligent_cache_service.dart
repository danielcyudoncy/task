...
class IntelligentCacheService extends GetxService {
  ...
  Future<void> initialize() async {
    debugPrint(
        'IntelligentCacheService: Initializing intelligent cache service');
    ...
    debugPrint(
        'IntelligentCacheService: Intelligent cache service initialized');
  }
  ...
  Future<T?> get<T>(
    ...
    if (!forceRefresh) {
      ...
      if (cachedData != null) {
        ...
        debugPrint(
            'IntelligentCacheService: Persistent cache hit for $cacheKey');
    ...
    if (fallback != null) {
      try {
        ...
        debugPrint(
            'IntelligentCacheService: Fallback failed for $cacheKey: $e');
      }
    }
  ...
  Future<void> set<T>(
    ...
    final expiry =
        customExpiry ?? _cacheExpiryTimes[category] ?? const Duration(hours: 1);
    ...
    debugPrint(
        'IntelligentCacheService: Cached data for $cacheKey (expires in ${expiry.inMinutes} minutes)');
  }
  ...
  void _evictOldestEntry() {
    ...
    for (final entry in _memoryCache.entries) {
      ...
      if (oldestTime == null ||
          (accessTime != null && accessTime.isBefore(oldestTime))) {
  ...
  Future<dynamic> _getPersistentCache(
      String key, Duration? customExpiry) async {
    ...
    try {
      ...
      debugPrint(
          'IntelligentCacheService: Error reading persistent cache for $key: $e');
      return null;
    }
  }

  Future<void> _setPersistentCache(
      String key, dynamic data, Duration expiry, bool compress) async {
    try {
      ...
      debugPrint(
          'IntelligentCacheService: Error storing persistent cache for $key: $e');
    }
  }
  ...
  T? _deserializeData<T>(dynamic data) {
    ...
    try {
      ...
      debugPrint(
          'IntelligentCacheService: Deserialization error for type $T: $e');
      return data as T?;
    }
  }
  ...
  Future<void> _cleanupExpiredEntries() async {
    ...
    if (expiredKeys.isNotEmpty) {
      debugPrint(
          'IntelligentCacheService: Cleaned up ${expiredKeys.length} expired entries');
    }
  }
