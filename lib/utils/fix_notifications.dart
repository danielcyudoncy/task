...
class NotificationFixer {
  static Future<void> fixExistingNotifications() async {
    try {
      ...
      debugPrint('NotificationFixer: Starting to fix notifications for user $uid');
      ...
      debugPrint('NotificationFixer: Found ${notificationsSnapshot.docs.length} notifications');
      ...
      for (final notifDoc in notificationsSnapshot.docs) {
        ...
        if (type == 'task_assignment' && (taskId == null || taskId.toString().isEmpty)) {
          debugPrint('NotificationFixer: Found task_assignment notification without taskId');
          ...
          if (tasksSnapshot.docs.isNotEmpty) {
            ...
            debugPrint('NotificationFixer: Updating notification with taskId: $newTaskId');
            ...
            debugPrint('NotificationFixer: Successfully updated notification ${notifDoc.id}');
          } else {
            debugPrint('NotificationFixer: No matching tasks found for notification ${notifDoc.id}');
          }
        }
        
        // Also ensure all notifications have the isRead field (not just 'read')
        if (!data.containsKey('isRead') && data.containsKey('read')) {
          debugPrint('NotificationFixer: Converting read field to isRead for notification ${notifDoc.id}');
          await FirebaseFirestore.instance
              .collection('users')
              .doc(uid)
              .collection('notifications')
