...
class PerformanceController extends GetxController {
    ...
    try {
      ...
      for (var userDoc in users) {
        ...
        final performanceData = {
          ...
          'performanceGrade':
              _calculatePerformanceGrade(userMetrics['completionRate']),
        };
      ...
      performanceList.sort((a, b) => (b['completionRate'] as double)
          .compareTo(a['completionRate'] as double));

      // Compute alternative averages
      final withAssigned = performanceList
          .where((u) => (u['totalAssignedTasks'] as int) > 0)
          .toList();
      final totalAssignedAll = withAssigned.fold<int>(
          0, (acc, u) => acc + (u['totalAssignedTasks'] as int));
      final totalCompletedAll = withAssigned.fold<int>(
          0, (acc, u) => acc + (u['completedTasks'] as int));
      final weightedAvg = totalAssignedAll > 0
          ? (totalCompletedAll / totalAssignedAll) * 100.0
          : 0.0;
      final meanExcludingZero = withAssigned.isNotEmpty
          ? withAssigned.fold<double>(
                  0.0, (acc, u) => acc + (u['completionRate'] as double)) /
              withAssigned.length
          : 0.0;
  ...
  Map<String, dynamic> _calculateUserMetrics(
      String userId, String userName, List<Task> allTasks) {
    ...
    final assignedTasks = allTasks
        .where((task) =>
            ...
            (task.assignedReporter != null &&
                task.assignedReporter == userName) ||
            (task.assignedCameraman != null &&
                task.assignedCameraman == userName) ||
    ...
    final overdueTasks = inProgressTasks
    ...
    for (final task in completedTasks) {
      final due =
          task.dueDate; // using due date as the end target within quarter
      if (due != null) {
        ...
        final completionTime =
            task.userCompletionTimestamps[userId] ?? task.timestamp;
    ...
    final double quarterMark =
        totalCompleted > 0 ? sumTaskMarks / totalCompleted : 0.0;
    ...
    final recentCompletedTasks = completedTasks
        .where((task) =>
  ...
  List<Map<String, dynamic>> getUsersByPerformanceGrade(String grade) {
    return userPerformanceData
        .where((user) => user['performanceGrade'] == grade)
        .toList();
  }
